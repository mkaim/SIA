...
class Map:
	...
	def __init__(self, rowNum, colNum, display):
		...
		self.map[4][2] = Field(Field.STORAGE_TYPE, [Crate(6, 3), Crate(772, 2)])
		self.map[3][2] = Field(Field.STORAGE_TYPE, [Crate(7, 3)])
		self.map[2][3] = Field(Field.CRANE_TYPE, [Crane(1, (2, 3), 3, 2, 10, [], self)])
		self.map[3][5] = Field(Field.CRANE_TYPE, [Crane(2, (3, 5), 2, 1, 10, [], self)])
